...
#   ret = unlist(lapply(json,jsonToAttrSub),recursive = FALSE)
#   return(ret)
# }
# 
# jsonToAttrSub <- function(json){
#   ret <- unlist(lapply(json$children, jsonToAttrSub),recursive=FALSE)
#   attr_list = list(json$state$selected,json$state$disabled,json$state$opened)
#   names(attr_list) <- list("stselected","stdisabled","stopened")
#   ret[[json$text]] <- attr_list
#   return(ret)
# }
